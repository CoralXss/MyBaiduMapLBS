...
public class LBSCloudSearch {
	...
	//云检索公钥
	private static String ak = "3sYcmkhEGZL0EAx0rGI5SPOu";
	
	private static String geotable_id = "119162";
		...
		new Thread() {
			public void run() {
				...
				while (count > 0){
					try {
						...
						if ( status == HttpStatus.SC_OK ) {
							...
							Message msgTmp = handler.obtainMessage(Constant.MSG_NET_SUCC);
							msgTmp.obj = result;
							msgTmp.sendToTarget();
							...
							httpRequest.abort();
							Message msgTmp = handler.obtainMessage(Constant.MSG_NET_STATUS_ERROR);
				...
				if ( count <= 0 && handler != null){
					Message msgTmp =  handler.obtainMessage(Constant.MSG_NET_TIMEOUT);
					msgTmp.sendToTarget();
				}
